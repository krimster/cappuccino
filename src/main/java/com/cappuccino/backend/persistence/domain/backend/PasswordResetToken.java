...
import com.cappuccino.backend.persistence.converters.LocalDateTimeAttributeConverter;
...
public class PasswordResetToken implements Serializable {
     ...
     * @throws IllegalArgumentException If the the token, user or creation date time are null
     */
    public PasswordResetToken(String token, User user, LocalDateTime creationDateTime, int expirationInMinutes) {
        ...
        if ((null == token) || (null == user) || (null == creationDateTime)) {
            throw new IllegalArgumentException("token, user and creation date time cannot be nul");
        }
