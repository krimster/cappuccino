...
import com.cappuccino.backend.service.PasswordResetTokenService;
...
public class ForgotMyPasswordController {
    ...
    @RequestMapping(method = RequestMethod.GET, value = FORGOT_PASSWORD_URL_MAPPING)
    ...
    public String forgotPasswordPost(HttpServletRequest request,
        ...
        if (null == passwordResetToken) {
            ...
            mailMessage.setSubject("[Cappuccino]: How to Reset Your Password");
            mailMessage.setText(emailText + "\r\n" + resetPasswordUrl);
            mailMessage.setFrom(webMasterEmail);

            //send email
            emailService.sendGenericMessage(mailMessage);
        }
    ...
    public String changeUserPasswordGet(@RequestParam("id") long id,
        ...
        if (StringUtils.isEmpty(token) || id == 0) {
            LOG.error("Invalid user id {} or token value {}", token);
